...
app.controller('Searchcontroller',function($scope,solrstorage,$log,$http){
	...
	promise.then(function(response){
		...
		$scope.docTypeFacet = facetArrayToObj(response.facet_counts.facet_fields.source);
		$scope.kbTypeFacet = facetArrayToObj(response.facet_counts.facet_fields.version);
		$scope.productFacet = facetArrayToObj(response.facet_counts.facet_fields.product);
	...
	$scope.createFQ = function(){
		var filters = 0;
		var filterQuery = '';
		if(createFilterQuery("{!tag=src}source", $scope.selectedDTFacet) != ''){
			filterQuery = filterQuery + createFilterQuery("{!tag=src}source", $scope.selectedDTFacet);
			filters++;
		}
		if(createFilterQuery("{!tag=ver}version", $scope.selectedKBFacet) != ''){
			if(filters == 1){
				filterQuery = filterQuery + " AND "+createFilterQuery("{!tag=ver}version", $scope.selectedKBFacet);
				filters++;
			} else{
				filterQuery = filterQuery+createFilterQuery("{!tag=ver}version", $scope.selectedKBFacet);
				filters++;
			}
		}
		if(createFilterQuery("{!tag=pro}product", $scope.selectedProFacet) != ''){
			if(filters == 1 || filters == 2){
				filterQuery = filterQuery + " AND "+createFilterQuery("{!tag=pro}product", $scope.selectedProFacet);
			} else{
				filterQuery = filterQuery + createFilterQuery("{!tag=pro}product", $scope.selectedProFacet);
			}
