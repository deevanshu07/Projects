...
<script>
	...
	function func2(object) {
		...
		$(object).closest("div").next("div").show();
	}
	...
	function func(result) {
		...
		if ($(result).attr('class').match('fa-angle-right')) {
			$(result).removeClass('fa-angle-right');
			$(result).addClass('fa-angle-down');
			$(result).closest('div').children('ul').removeClass("p-hide");
			$(result).closest('div').children('ul').addClass("p-open");
		} else {
			$(result).removeClass('fa-angle-down');
			$(result).addClass('fa-angle-right');
			$(result).closest('div').children('ul').removeClass("p-open");
			$(result).closest('div').children('ul').addClass("p-hide");
		}
...
<body data-ng-app="solr" data-ng-controller="Searchcontroller">
				...
				<div class="filter_area filter_partition_area">
					...
					<div class="merge_area">
						...
						<input type="text" class="form-control" id="usr"
							...
							data-ng-keyup="onkeypress($event.keyCode) || suggestion()  || getSuggestions()"
						...
						-->
						<button type="button"
							class="search_btn  btn-cust-style btn-primary"
							data-ng-click="searching()">
							<span class="glyphicon glyphicon-search fa fa-search"></span>
						</button>
						</span>
						<div class="search-sug"
							data-ng-show="search != '' && focus == true">
							<div data-ng-repeat="facet in searchSuggestion"
								data-ng-show="facet.val !=0"
								data-ng-class="{'back-cust': $index == count}"
								data-ng-mouseleave="setCount(-1)"
								data-ng-click="setSearchKeyword(facet)">
								<span style="padding-left: 5px; font-size: 16px;"> <span
									data-ng-class=" facet.val != 0 ? 'cust-color-blue' : 'cust-color-gray'">{{facet}}</span>
									<span class="pull-right" data-ng-show="$index == 0"
									style="font-size: 12px; padding-right: 5px; font-weight: 400">search
	...
	<div class="content_container">
		...
		<div class="right_left_panel ">
			<div class="left_panel fadeInLeft panelslide" id="slider"
				style="overflow: auto;">
...
<label for="test1">All</label></h3> -->
						...
						<h3 id="acc-header-1" class="accord">
							<input type="checkbox" class="all_input" value="" name="level-1"
								id="test1" data-ng-click="fetchResultForFacet('')"
								data-ng-checked="facet == ''"> <label for="test1">All
								Sources</label>
					...
					<div data-ng-repeat="fa in productFacet">
						<h3 id="acc-header" class="accord">
							<a name="level-1"
								id="test2" /> <label for="test2">{{fa.field}}</label> <i
								class="fa fa-angle-down " aria-hidden="true"></i></a>
						</h3>
						<ul class="p-open accord">
							<label class="labelStyle" data-ng-repeat="facet in fa.children | limitTo:3">
								<li ng-class="{anchorActive1: config.params.filterQuery1.indexOf(facet.name)>-1}" ><a style="cursor: pointer;"
									ng-click="insertKBFacet(facet.name,fa.field)"> <label
									for="{{facet.name}}"></label><span
									 ng-class="{anchorActive: config.params.filterQuery1.indexOf(facet.name)>-1}">{{facet.name}}
										({{facet.count}})</span></a></li>
							</label>
						</ul>
					</div>
    ...
    height: 550px">
				...
				<div class="acc-toggle" >
					<div>
						...
						<h3>
							<input type="checkbox" value="" name="level-{{$index}}"
								id="test{{$index}}" ng-click="fetchResultForFacet('')">
					...
					<div class="panel panel-default" data-ng-repeat="fa in productFacet" >
						
						<div class="panel-heading"><h3>{{fa.field}}</h3></div>
						


						<div class="list-group">
							<a style="cursor: pointer; margin:1px"
								ng-click="insertKBFacet(facet.name,fa.field)"
								class="list-group-item"
								ng-class="{anchorActive: config.params.filterQuery.indexOf(facet.name)>-1}"
								data-ng-repeat="facet in fa.children | limitTo:5"> <h4>{{facet.name}}
								({{facet.count}})</h4></a>
								
								 <a style="cursor: pointer"
								ng-click="filterCollapsed[$index] = !filterCollapsed[$index]" onclick="func2(this)"
								class="list-group-item"
								ng-if="fa.children.length>5 && filterCollapsed[$index]"><h4>+
								More</h4></a>

						</div>
						<div class="list-group" 
							ng-if="fa.children.length>5" style="display:none">
							
							<a style="cursor: pointer ; margin:1px"
								data-ng-click="insertKBFacet(val.name,fa.field)"
								class="list-group-item"
								data-ng-repeat="val in fa.children | filter: validSearchFilter | limitTo: 5-fa.children.length"
								ng-class="{anchorActive: config.params.filterQuery.indexOf(val.name)>-1}"><h4>{{val.name}}
								<span class="badge">({{val.count}})</span></h4>
								 </a> <a style="cursor: pointer"
								ng-click="filterCollapsed[$index] = !filterCollapsed[$index]" onclick="func3(this)"
								class="list-group-item"><h4>- Less</h4></a>
			...
			<div class="right_panel panelslide_right" id="right_panel_slide">
				<div class="right_panel_top">
					...
					<div class="content_header_panel">
						<div class="search_text">
							...
							<p class="didM">
								<span data-ng-show="numFound == 0 && !correctlySpelled"
									style="font-size: 16px; color: #636c72;">Did You mean <span
									class="spellcheck-word-style"
									data-ng-click="setSearchKeywordAndSearch(suggestedWord)">{{suggestedWord}}</span></span>
							</p>
						</div>
					...
					<div class="content_header_panel_right">
						...
						<div class="sort_area">
							...
							<ul>
								<li id="mp"><a href="#" data-ng-model="sortOrder"
									data-ng-click="sortingNew('kudosCount_desc')">With ML</a></li>
								<li id="mr"><a href="#" data-ng-model="sortOrder"
									data-ng-click="sortingNew('score_desc')">Without ML</a></li>
							</ul>
						</div>
				...
				<div class="panel_area">
					...
					<div class="right_panel_box grid_view zoomIn {{doc.cssClass}}"
						data-ng-repeat="doc in documentList">
						<span class="box-date">Updated on {{doc.last_edit_time |
							date: 'yyyy-MM-dd' }} </span> <a href="articleDetail.jsp?id={{doc.id}}"
							data-ng-if="!(doc.id | isUrl)" target="_blank"
							style="text-decoration: none"><h2 class="window_heading"
								data-ng-bind-html="highlight | extractTitle:doc.id:doc.title"
								data-ng-click="clickThrough($index+1+start,doc.title,doc.id,101)"></h2></a>
						<p class="meta">
							<span class="_mb" ng-show='{{doc["[elevated]"]}}'> Ad</span>
							<!-- <b>Forum:</b>
								<span  data-ng-repeat ="x in doc.label">{{x}}{{$last ? '' : ', '}}</span> | -->
							<b>Kudos:</b> {{doc.kudosCount}} | <span class="_mb"
								ng-show='{{doc["[elevated]"]}}'> Ad</span> <b>Views:</b>
							{{doc.viewsCount}}
						</p>
...
<p><span>Administration Guide in <span>Product Documentation</span></p>
 ...
 -->
				...
				<div class="row" data-ng-show="numFound != 0 && numFound > rows"">
					...
					<ul class="pagination">
						<li data-ng-class="{'disabled': range == 1}"
							data-ng-show="range != 1"><a class="pagerStyle"
							data-aria-label="Previous" data-ng-click="range = range - 10">
								<span data-aria-hidden="true">&laquo;</span>
						</a></li>
						<li data-ng-repeat="n in [] | range:range:numFound:rows"
							data-ng-class="{'active': (n-1) == start/rows}"><a
							class="pagerStyle" data-ng-click="pagination(n)">{{n}}</a></li>
						<li data-ng-class="{'disabled': (range+9)*rows >= numFound}"
							data-ng-hide="(range+9)*rows >= numFound"><a
							class="pagerStyle" data-aria-label="Next"
							data-ng-click="range = range + 10"> <span
								data-aria-hidden="true">&raquo;</span>
						</a></li>
					</ul>
					...
					<div class="pull-right right_nav" data-ng-show="numFound != 0">
						<div style="margin-top: 15px;">
							...
							<div style="margin-top: 10px">
								<label for="sel1" class="sel1">Result Per Page :</label> <select
									class="form-control" id="sel1" class="sel1"
									data-ng-model="rows" data-ng-change="resultPerPage()">
									<option value="10" data-ng-selected="rows == 10">10</option>
									<option value="25" data-ng-selected="rows == 25">25</option>
									<option value="50" data-ng-selected="rows == 50">50</option>
									<option value="100" data-ng-selected="rows == 100">100</option>
