...
<label for="test1">All</label></h3> -->
						...
						<div>
							<h3 id="acc-header-3" class="accord">
								<input class="secound_input" type="checkbox" name="level-3"
									id="test3" /> <label for="test3">Genres</label><i
									class="fa  fa-angle-right " aria-hidden="true"></i>
							</h3>
				...
				<div class="right_panel panelslide_right" id="right_panel_slide">
					...
					<div class="panel_area">
						...
						<span class="box-date">Released on {{doc.release_date | date: 'MM-dd-yyyy' }} </span>
							<a href="NewarticleDetail.jsp?id={{doc.id}}" data-ng-if="!(doc.id | isUrl)"
							...
							<p class="meta">
								...
								<span  data-ng-repeat ="x in doc.label">{{x}}{{$last ? '' : ', '}}</span> | --> <b>Rating:</b> {{doc.vote_average}} |
								<span class="_mb" ng-show='{{doc["[elevated]"]}}'> Ad</span> <b>Views:</b> {{doc.viewsCount}}
							</p>
							<p
								data-ng-bind-html="highlight | extractContent:doc.docId:doc.body.content | limitChar">{{doc.body.content[4]}}</p>
